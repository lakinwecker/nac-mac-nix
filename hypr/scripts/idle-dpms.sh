...
#
# Under a Lua config `hyprctl dispatch` parses its argument as Lua (see
# hypr-lua.sh), so the old `hyprctl dispatch dpms off` dies with "')' expected
# near 'off'" and the whole listener silently does nothing. The working
# spelling is a Lua expression, `hl.dsp.dpms("off")`, whose quotes and parens
# do not survive the `sh -c` that hypridle wraps every command in. One script
# instead, so the quoting lives in exactly one place.
#
